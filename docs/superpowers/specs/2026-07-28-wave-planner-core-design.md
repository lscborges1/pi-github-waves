...
**Phase 3 — edges and closure:** edge checks use this precedence table. A lower row never runs for a group rejected by a higher row.

| Order | Group/check | Result |
|---:|---|---|
| 1 | Group original edges by exact `(blockerNodeId, blockedNodeId, source)`, sorted lexicographically by those fields | Establish one deterministic exact group; no diagnostic yet |
| 2 | Either endpoint of an exact group is absent | One `edge_endpoint_missing` per exact group; no duplicate, conflict, self, or count diagnostics for it |
| 3 | Both endpoints resolve and blocker equals blocked | One `self_dependency` per endpoint pair; no duplicate, conflict, or count diagnostics |
| 4 | A remaining endpoint pair has both `native` and `body` exact groups | One `duplicate_edge_conflict`; no duplicate warning or dependency-count contribution |
| 5 | One-source exact group has two or more original occurrences | One `duplicate_edge`, then collapse to one edge |
| 6 | Count normalized edges | For each blocked node, count distinct blocker IDs; emit one `dependency_limit_exceeded` if over 100 |

Examples: three duplicate missing-endpoint edges emit one missing-endpoint error and no warning; two missing-endpoint edges with different sources emit two missing-endpoint errors; conflicting-source self-edges emit only self-dependency; duplicate valid edges from one source emit one warning.
...
- `cycleIssueNumbers` is required only when `origin: "core"`, `code: "dependency_cycle"`, and severity is `error`; then it has at least two unique ascending positive issue numbers. It is forbidden on every other diagnostic.
- `ready` requires level 1; `blocked_selected` requires an integer level >=2; all other dispositions require null level.
- direct and unresolved blocker arrays are unique ascending positive issue numbers, and unresolved is a subset of direct.
...
Canonicalizer tests cover every prohibited value, cyclic roots and nested cycles, non-plain objects, missing top-level and nested fields, unknown top-level and nested properties, wrong arrays/scalars, cycle-diagnostic constraints, disposition/level constraints, batch partition and size constraints, blocker subset/order constraints, deterministic first-error paths, and exact `TypeError` messages. Diagnostic tests include source/core code collisions and verify origin-aware sorting and fingerprints.
